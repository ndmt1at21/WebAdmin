...
var ServerSelectionError = require('./error/serverSelection');
...
var getDefaultBulkwriteResult = require('./helpers/getDefaultBulkwriteResult');
...
var isDefaultIdIndex = require('./helpers/indexes/isDefaultIdIndex');

var isIndexEqual = require('./helpers/indexes/isIndexEqual');
...
var removeDeselectedForeignField = require('./helpers/populate/removeDeselectedForeignField');
...
Model.prototype.$__handleSave = function (options, callback) {
  ...
  if (this.isNew) {
    ...
    if (delta) {
      ...
      var optionsWithCustomValues = Object.assign({}, options, saveOptions);
      this.constructor.exists(this.$__where(), optionsWithCustomValues).then(function (documentExists) {
        if (!documentExists) {
          throw new DocumentNotFoundError(_this2.$__where(), _this2.constructor.modelName);
        }
    ...
    if (get(options, 'safe.w') !== 0 && get(options, 'w') !== 0) {
      ...
      if (_this3.$__.version && !_this3.$__.inserting) {
        ...
        if (numAffected <= 0) {
          ...
          var err = _this3.$__.$versionError || new VersionError(_this3, version, _this3.$__.modifiedPaths);
          return callback(err);
...
Model.prototype.save = function (options, fn) {
  ...
  return this.db.base._promiseOrCallback(fn, function (cb) {
...
Model.prototype.remove = function remove(options, fn) {
  ...
  return this.db.base._promiseOrCallback(fn, function (cb) {
    ...
    _this5.$__remove(options, function (err, res) {
      _this5.$op = null;
      cb(err, res);
    });
  }, this.constructor.events);
...
Model.init = function init(callback) {
  ...
  var autoCreate = utils.getOption('autoCreate', this.schema.options, this.db.config, this.db.base.options);
...
Model.createCollection = function createCollection(options, callback) {
  ...
  return this.db.base._promiseOrCallback(callback, function (cb) {
        ...
        for (var _iterator2 = indexes[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
          ...
          try {
            for (var _iterator3 = schemaIndexes[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
              ...
              var options = _decorateDiscriminatorIndexOptions(_this11, utils.clone(schemaIndex[1]));
...
function _decorateDiscriminatorIndexOptions(model, indexOptions) {
  ...
  if (model.baseModelName != null && !('partialFilterExpression' in indexOptions) && !('sparse' in indexOptions)) {
    ...
    indexOptions.partialFilterExpression = _defineProperty({}, discriminatorKey, value);
  }
...
Model.translateAliases = function translateAliases(fields) {
  ...
  var translate = function translate(key, value) {
    ...
    for (var _i in fieldKeys) {
      var name = fieldKeys[_i];
      ...
      for (var _i2 = 0, _Object$keys = Object.keys(fields); _i2 < _Object$keys.length; _i2++) {
        ...
        if (key[0] === '$') {
          if (Array.isArray(fields[key])) {
            for (var _i3 in fields[key]) {
              // Recursively translate nested queries
              fields[key][_i3] = this.translateAliases(fields[key][_i3]);
            }
          }
        }
      }
...
Model.create = function create(doc, options, callback) {
  ...
  return this.db.base._promiseOrCallback(cb, function (cb) {
...
Model.watch = function (pipeline, options) {
  ...
  var changeStreamThunk = function changeStreamThunk(cb) {
    if (_this16.collection.buffer) {
      _this16.collection.addQueue(function () {
        if (_this16.closed) {
          return;
        }

        var driverChangeStream = _this16.collection.watch(pipeline, options);
...
Model.insertMany = function (arr, options, callback) {
  ...
  return this.db.base._promiseOrCallback(callback, function (cb) {
    _this17.$__insertMany(arr, options, cb);
  }, this.events);
};
/*!
 * ignore
  ...
  parallelLimit(toExecute, limit, function (error, docs) {
    ...
    _this.collection.insertMany(docObjects, options, function (error, res) {
      if (error) {
        ...
        if (error.writeErrors == null && get(error, 'result.result.writeErrors') != null) {
          error.writeErrors = error.result.result.writeErrors;
        } // `insertedDocs` is a Mongoose-specific property


        var erroredIndexes = new Set(get(error, 'writeErrors', []).map(function (err) {
          return err.index;
        }));
        error.insertedDocs = docAttributes.filter(function (doc, i) {
      ...
      try {
        for (var _iterator6 = docAttributes[Symbol.iterator](), _step6; !(_iteratorNormalCompletion6 = (_step6 = _iterator6.next()).done); _iteratorNormalCompletion6 = true) {
...
function _setIsNew(doc, val) {
  ...
  try {
    for (var _iterator7 = subdocs[Symbol.iterator](), _step7; !(_iteratorNormalCompletion7 = (_step7 = _iterator7.next()).done); _iteratorNormalCompletion7 = true) {
      var subdoc = _step7.value;
      subdoc.isNew = val;
    }
  } catch (err) {
...
Model.bulkWrite = function (ops, options, callback) {
  ...
  var validations = ops.map(function (op) {
    return castBulkWrite(_this18, op, options);
  });
  callback = this.$handleCallbackError(callback);
  return this.db.base._promiseOrCallback(callback, function (cb) {
    ...
    each(validations, function (fn, cb) {
      ...
      _this18.collection.bulkWrite(ops, options, function (error, res) {
        if (error) {
          return cb(error);
        }
...
Model.hydrate = function (obj, projection) {
  ...
  var model = require('./queryhelpers').createModel(this, obj, projection);
...
Model.mapReduce = function mapReduce(o, callback) {
  ...
  return this.db.base._promiseOrCallback(callback, function (cb) {
    ...
    if (o.query) {
      var q = new _this19.Query(o.query);
      q.cast(_this19);
      o.query = q._conditions;
      q = undefined;
    }

    _this19.collection.mapReduce(null, null, o, function (err, res) {
      if (err) {
        return cb(err);
      }
      ...
      if (res.collection) {
        ...
        var model = Model.compile('_mapreduce_' + res.collection.collectionName, Model.mapReduce.schema, res.collection.collectionName, _this19.db, _this19.base);
  ...
  return this.db.base._promiseOrCallback(callback, function (cb) {
    ...
    if (pathsToValidate != null) {
      ...
      paths = paths.filter(function (p) {
        ...
        try {
          for (var _iterator8 = pieces[Symbol.iterator](), _step8; !(_iteratorNormalCompletion8 = (_step8 = _iterator8.next()).done); _iteratorNormalCompletion8 = true) {
    ...
    try {
      var _loop = function _loop() {
        ...
        for (var _i6 = 0; _i6 < pieces.length - 1; ++_i6) {
          cur = cur[pieces[_i6]];
        }
        ...
        schemaType.doValidate(val, function (err) {
          if (err) {
            ...
            if (err instanceof ValidationError) {
              for (var _i7 = 0, _Object$keys2 = Object.keys(err.errors); _i7 < _Object$keys2.length; _i7++) {
                var _err = _Object$keys2[_i7];
    ...
    function pushNestedArrayPaths(nestedArray, path) {
      ...
      for (var _i5 = 0; _i5 < nestedArray.length; ++_i5) {
        if (Array.isArray(nestedArray[_i5])) {
          pushNestedArrayPaths(nestedArray[_i5], path + '.' + _i5);
        } else {
          paths.push(path + '.' + _i5);
        }
      }
...
Model.geoSearch = function (conditions, options, callback) {
  ...
  return this.db.base._promiseOrCallback(callback, function (cb) {
    ...
    _this21.collection.geoHaystackSearch(options.near[0], options.near[1], options, function (err, res) {
      if (err) {
        return cb(err);
      }
      ...
      for (var _i8 = 0; _i8 < res.results.length; ++_i8) {
        var temp = res.results[_i8];
        res.results[_i8] = new _this21();

        res.results[_i8].init(temp, {}, init);
      }
    });
  }, this.events);
};
...
function populate(model, docs, options, callback) {
    ...
    if (mod.foreignField.size === 1) {
      ...
      try {
        for (var _iterator12 = mod.foreignField[Symbol.iterator](), _step12; !(_iteratorNormalCompletion12 = (_step12 = _iterator12.next()).done); _iteratorNormalCompletion12 = true) {
          var _foreignField = _step12.value;
  ...
  if (!hasOne) {
    ...
    if (options.populate != null) {
      var opts = utils.populate(options.populate).map(function (pop) {
  ...
  for (var _i10 = 0, _params = params; _i10 < _params.length; _i10++) {
    var arr = _params[_i10];
  ...
  function _done() {
    ...
    try {
      for (var _iterator13 = params[Symbol.iterator](), _step13; !(_iteratorNormalCompletion13 = (_step13 = _iterator13.next()).done); _iteratorNormalCompletion13 = true) {
        ...
        try {
          for (var _iterator15 = vals[Symbol.iterator](), _step15; !(_iteratorNormalCompletion15 = (_step15 = _iterator15.next()).done); _iteratorNormalCompletion15 = true) {
            var val = _step15.value;

            _mod.options._childDocs.push(val);
          }
        } catch (err) {
    ...
    try {
      for (var _iterator14 = params[Symbol.iterator](), _step14; !(_iteratorNormalCompletion14 = (_step14 = _iterator14.next()).done); _iteratorNormalCompletion14 = true) {
        var _arr = _step14.value;
        removeDeselectedForeignField(_arr[0].foreignField, _arr[0].options, vals);
      }
    } catch (err) {
...
function _execPopulateQuery(mod, match, select, assignmentOpts, callback) {
  ...
  try {
    for (var _iterator16 = mod.foreignField[Symbol.iterator](), _step16; !(_iteratorNormalCompletion16 = (_step16 = _iterator16.next()).done); _iteratorNormalCompletion16 = true) {
      var _foreignField2 = _step16.value;
  ...
  if (mod.count) {
    ...
    try {
      for (var _iterator17 = mod.foreignField[Symbol.iterator](), _step17; !(_iteratorNormalCompletion17 = (_step17 = _iterator17.next()).done); _iteratorNormalCompletion17 = true) {
        var foreignField = _step17.value;
  ...
  query.exec(function (err, docs) {
    ...
    try {
      for (var _iterator18 = docs[Symbol.iterator](), _step18; !(_iteratorNormalCompletion18 = (_step18 = _iterator18.next()).done); _iteratorNormalCompletion18 = true) {
...
function _assign(model, vals, mod, assignmentOpts) {
  ...
  for (var _i11 = 0; _i11 < len; _i11++) {
    ...
    try {
      for (var _iterator19 = mod.foreignField[Symbol.iterator](), _step19; !(_iteratorNormalCompletion19 = (_step19 = _iterator19.next()).done); _iteratorNormalCompletion19 = true) {
        var foreignField = _step19.value;
        ...
        if (Array.isArray(_val)) {
          ...
          try {
            for (var _iterator20 = _val[Symbol.iterator](), _step20; !(_iteratorNormalCompletion20 = (_step20 = _iterator20.next()).done); _iteratorNormalCompletion20 = true) {
              var __val = _step20.value;
              ...
              if (rawDocs[key]) {
                if (Array.isArray(rawDocs[key])) {
                  ...
                  rawOrder[key].push(_i11);
                } else {
                  rawDocs[key] = [rawDocs[key], val];
                  rawOrder[key] = [rawOrder[key], _i11];
                }
              } else {
...
function applyQueryMethods(model, methods) {
  for (var _i12 in methods) {
    model.Query.prototype[_i12] = methods[_i12];
  }
}
/*!
...
Model.__subclass = function subclass(conn, schema, collection) {
  ...
  if (_this.discriminators != null) {
    ...
    for (var _i13 = 0, _Object$keys3 = Object.keys(_this.discriminators); _i13 < _Object$keys3.length; _i13++) {
...
Model.$wrapCallback = function (callback) {
  var serverSelectionError = new ServerSelectionError();
