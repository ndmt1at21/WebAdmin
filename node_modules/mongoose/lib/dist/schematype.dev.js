...
SchemaType.prototype._applySetters = function (value, scope, init, priorVal) {
  ...
  try {
    for (var _iterator = utils.clone(setters).reverse()[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
  ...
  if (Array.isArray(v) && caster && caster.setters) {
    ...
    for (var i = 0; i < v.length; ++i) {
      ...
      try {
        newVal.push(caster.applySetters(_value, scope, init, priorVal));
      } catch (err) {
...
SchemaType.prototype.castForQueryWrapper = function (params) {
  ...
  if ('$conditional' in params) {
    var _ret = this.castForQuery(params.$conditional, params.val);
  ...
  if (params.$skipQueryCastForUpdate || params.$applySetters) {
    var _ret2 = this._castForQuery(params.val);
