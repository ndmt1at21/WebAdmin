...
Object.defineProperty(Connection.prototype, 'readyState', {
  ...
  set: function set(val) {
    ...
    if (this._readyState !== val) {
      ...
      try {
        for (var _iterator = this.otherDbs[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
          var db = _step.value;
          db.readyState = val;
...
Connection.prototype.transaction = function transaction(fn) {
  return this.startSession().then(function (session) {
    ...
    return session.withTransaction(function () {
      ...
      try {
        for (var _iterator2 = session[sessionNewDocuments].keys()[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
          var doc = _step2.value;
          var state = session[sessionNewDocuments].get(doc);
          ...
          try {
            for (var _iterator3 = state.modifiedPaths[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
          ...
          try {
            for (var _iterator4 = state.atomics.keys()[Symbol.iterator](), _step4; !(_iteratorNormalCompletion4 = (_step4 = _iterator4.next()).done); _iteratorNormalCompletion4 = true) {
              var _path = _step4.value;
              var val = doc.$__getValue(_path);

              if (val == null) {
                continue;
              }

              val[arrayAtomicsSymbol] = state.atomics.get(_path);
            }
          } catch (err) {
...
function _wrapConnHelper(fn) {
  return function () {
    ...
    return promiseOrCallback(cb, function (cb) {
      ...
      // Re: gh-8534
      immediate(function () {
        if (_this2.readyState === STATES.connecting && _this2._shouldBufferCommands()) {
...
Connection.prototype.openUri = function (uri, options, callback) {
  ...
  this.then = function (resolve, reject) {
    return this.$initialConnection.then(function () {
...
function _setClient(conn, client, options, dbName) {
  ...
  if (options.useUnifiedTopology) {
    if (type === 'Single') {
      ...
      client.on('topologyDescriptionChanged', function (ev) {
        // Emit disconnected if we've lost connectivity to _all_ servers
        // in the replica set.
        var description = ev.newDescription;
        var servers = Array.from(ev.newDescription.servers.values());
        var allServersDisconnected = description.type === 'ReplicaSetNoPrimary' && servers.reduce(function (cur, d) {
          return cur || d.type === 'Unknown';
        }, false);
...
Connection.prototype._close = function (force, callback) {
  ...
  var closeCalled = this._closeCalled;
...
Connection.prototype.watch = function (pipeline, options) {
  ...
  var disconnectedError = new MongooseError('Connection ' + this.id + ' was disconnected when calling `watch()`');

  var changeStreamThunk = function changeStreamThunk(cb) {
    immediate(function () {
      if (_this5.readyState === STATES.connecting) {
        _this5.once('open', function () {
          var driverChangeStream = this.db.watch(pipeline, options);
          cb(null, driverChangeStream);
        });
      } else if (_this5.readyState === STATES.disconnected && _this5.db == null) {
        cb(disconnectedError);
      } else {
        var driverChangeStream = _this5.db.watch(pipeline, options);
  ...
  var changeStream = new ChangeStream(changeStreamThunk, pipeline, options);
  return changeStream;
};
/**
...
Connection.prototype.setClient = function setClient(client) {
  ...
  _setClient(this, client, {
    useUnifiedTopology: client.s.options.useUnifiedTopology
