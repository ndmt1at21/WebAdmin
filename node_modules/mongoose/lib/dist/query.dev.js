...
var removeUnusedArrayFilters = require('./helpers/update/removeUnusedArrayFilters');
...
Query.prototype.__distinct = wrapThunk(function __distinct(callback) {
  ...
  var options = this._optionsForExec(); // don't pass in the conditions because we already merged them in
...
Query.prototype.findOneAndUpdate = function (criteria, doc, options, callback) {
  ...
  var returnOriginal = get(this, 'model.base.options.returnOriginal');
...
Query.prototype.exec = function exec(op, callback) {
  ...
  var _this = this; // Ensure that `exec()` is the first thing that shows up in
  // the stack when cast errors happen.


  var castError = new CastError();
  ...
  return promiseOrCallback(callback, function (cb) {
    ...
    _this10._hooks.execPre('exec', _this10, [], function (error) {
      ...
      var thunk = '_' + _this10.op;

      if (_this10.op === 'update') {
        thunk = '_execUpdate';
      } else if (_this10.op === 'distinct') {
        thunk = '__distinct';
      }

      _this10[thunk].call(_this10, function (error, res) {
        if (error) {
          return cb(_cleanCastErrorStack(castError, error));
        }
...
Query.prototype._castUpdate = function _castUpdate(obj, overwrite) {
  ...
  var baseSchema = schema._baseSchema ? schema._baseSchema : schema;

  if (this._mongooseOptions.overwriteDiscriminatorKey && obj[discriminatorKey] != null && baseSchema.discriminators) {
    var _schema = baseSchema.discriminators[obj[discriminatorKey]];
...
Query.prototype.populate = function () {
  ...
  if (this.options != null) {
    ...
    try {
      for (var _iterator2 = res[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
        var populateOptions = _step2.value;
  ...
  if (opts.lean != null) {
    ...
    try {
      for (var _iterator3 = res[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
        var _populateOptions = _step3.value;

        if (get(_populateOptions, 'options.lean') == null) {
          _populateOptions.options = _populateOptions.options || {};
          _populateOptions.options.lean = lean;
        }
      }
    } catch (err) {
  ...
  try {
    for (var _iterator4 = res[Symbol.iterator](), _step4; !(_iteratorNormalCompletion4 = (_step4 = _iterator4.next()).done); _iteratorNormalCompletion4 = true) {
      var _populateOptions2 = _step4.value;
      var path = _populateOptions2.path;

      if (pop[path] && pop[path].populate && _populateOptions2.populate) {
        _populateOptions2.populate = pop[path].populate.concat(_populateOptions2.populate);
      }

      pop[_populateOptions2.path] = _populateOptions2;
    }
  } catch (err) {
