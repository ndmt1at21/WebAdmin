...
function mangleScope(scope) {
  var newNames = new Set(); // Sort bindings so that more frequently referenced bindings get shorter names.

  var sortedBindings = Object.keys(scope.bindings).sort(function (a, b) {
    return scope.bindings[b].referencePaths.length - scope.bindings[a].referencePaths.length;
  });
...
function canRename(scope, binding, newName) {
  ...
  for (var i = 0; i < binding.referencePaths.length; i++) {
    var ref = binding.referencePaths[i];
