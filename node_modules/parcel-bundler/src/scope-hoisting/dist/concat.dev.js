...
var treeShake = require('./shake');

var mangleScope = require('./mangler');

var _require2 = require('./utils'),
    getName = _require2.getName,
    getIdentifier = _require2.getIdentifier;

var EXPORTS_RE = /^\$([^$]+)\$exports$/;
var ESMODULE_TEMPLATE = template("$parcel$defineInteropFlag(EXPORTS);");
var DEFAULT_INTEROP_TEMPLATE = template('var NAME = $parcel$interopDefault(MODULE)');
var THROW_TEMPLATE = template('$parcel$missingModule(MODULE)');
var REQUIRE_TEMPLATE = template('require(ID)');

module.exports = function (packager, ast) {
  var assets = packager.assets;
  var replacements = new Map();
  var imports = new Map();
  var referenced = new Set(); // Build a mapping of all imported identifiers to replace.
  ...
  try {
    for (var _iterator = assets.values()[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
      ...
      for (var name in asset.cacheData.imports) {
        var imp = asset.cacheData.imports[name];
  ...
  function replaceExportNode(module, originalName, path) {
    ...
    if (!node && !mod.cacheData.isCommonJS && mod.cacheData.isES6Module) {
      var relativePath = relative(packager.options.rootDir, mod.name);
      throw new Error("".concat(relativePath, " does not export '").concat(name, "'"));
  ...
  function interop(mod, originalName, path, node) {
    ...
    if (mod.cacheData.isCommonJS && originalName === 'default') {
      var name = getName(mod, '$interop$default');
      ...
      if (!path.scope.getBinding(name)) {
        ...
        var binding = path.scope.getBinding(getName(mod, 'exports'));
  ...
  traverse(ast, {
    CallExpression: function CallExpression(path) {
      ...
      if (callee.name === '$parcel$require') {
        ...
        var asset = assets.get(id.value);
        var mod = packager.resolveModule(id.value, source.value);
        ...
        if (!mod) {
          if (asset.dependencies.get(source.value).optional) {
            ...
            throw new Error("Cannot find module \"".concat(source.value, "\" in asset ").concat(id.value));
          }
        } else {
          var node;
          ...
          if (assets.get(mod.id)) {
                ...
                if (binding && !binding.path.getData('hasESModuleFlag')) {
                  ...
                  try {
                    for (var _iterator2 = binding.constantViolations[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
                      ...
                      _path.insertAfter(ESMODULE_TEMPLATE({
                        EXPORTS: name
                      }));
                    }
                  } catch (err) {
            ...
            if (mod.cacheData.shouldWrap) {
              var call = t.callExpression(getIdentifier(mod, 'init'), []);
        ...
        if (args.length !== 2 || !t.isStringLiteral(_id) || !t.isStringLiteral(_source)) {
          throw new Error('invariant: invalid signature, expected : $parcel$require$resolve(number, string)');
        }

        var mapped = assets.get(_id.value);
        var dep = mapped.dependencies.get(_source.value);

        var _mod = mapped.depAssets.get(dep);

        var bundles = _mod.id;

        if (dep.dynamic && packager.bundle.childBundles.has(_mod.parentBundle)) {
          bundles = [];
          ...
          try {
            for (var _iterator3 = _mod.parentBundle.siblingBundles[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
          ...
          bundles.push(packager.getBundleSpecifier(_mod.parentBundle));
          bundles.push(_mod.id);
        }
    ...
    VariableDeclarator: {
      exit: function exit(path) {
        ...
        var match = init.name.match(EXPORTS_RE);
    ...
    MemberExpression: {
      exit: function exit(path) {
        ...
        var match = object.name.match(EXPORTS_RE); // If it's a $id$exports.name expression.

        if (match) {
          var name = t.isIdentifier(property) ? property.name : property.value;
    ...
    ReferencedIdentifier: function ReferencedIdentifier(path) {
      ...
      if (imports.has(name)) {
        ...
        var node = replaceExportNode(imp[0], imp[1], path); // If the export does not exist, replace with an empty object.
      ...
      var match = name.match(EXPORTS_RE);
