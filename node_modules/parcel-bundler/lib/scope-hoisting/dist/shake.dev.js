...
function treeShake(scope) {
  ...
  do {
    ...
    Object.keys(scope.bindings).forEach(function (name) {
      var binding = getUnusedBinding(scope.path, name); // If it is not safe to remove the binding don't touch it.
...
function getUnusedBinding(path, name) {
  ...
  var bailout = binding.referencePaths.some(function (path) {
    return !isExportAssignment(path) && !isUnusedWildcard(path);
  });
