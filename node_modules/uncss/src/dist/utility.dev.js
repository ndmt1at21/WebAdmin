...
function readStylesheets(files, outputBanner) {
  return Promise.all(files.map(function (filename) {
    ...
    throw new Error("UnCSS: could not open ".concat(path.join(process.cwd(), filename)));
  })).then(function (res) {
    ...
    if (outputBanner) {
      for (var i = 0, len = files.length; i < len; i++) {
        ...
        var banner = "/*** uncss> filename: ".concat(files[i].replace(/\\/g, '/'), " ***/\n");
        res[i] = banner + res[i];
      }
    }
...
function parseErrorMessage(error, cssStr) {
  ...
  error.message = "uncss/node_modules/css: unable to parse ".concat(error.filename, ":\n").concat(error.message, "\n");
  return error;
}
