...
var dePseudify = function () {
  ...
  var processor = postcssSelectorParser(transform);
...
function filterUnusedSelectors(selectors, ignore, usedSelectors) {
  ...
  return selectors.filter(function (selector) {
    ...
    for (var i = 0, len = ignore.length; i < len; ++i) {
      ...
      if (/:\w+/.test(ignore[i])) {
        var ignored = dePseudify(ignore[i]);
  ...
  css.walkDecls(function (decl) {
    if (_.endsWith(decl.prop, 'animation-name')) {
      ...
      postcss.list.comma(decl.value).forEach(function (anim) {
        ...
        usedAnimations.push.apply(usedAnimations, _toConsumableArray(postcss.list.space(anim)));
      });
    }
  });
  var usedAnimationsSet = new Set(usedAnimations);
...
module.exports = function uncss(pages, css, ignore) {
  return Promise.all(pages.map(function (page) {
    ...
    var filteredCss = filterUnusedRules(css, ignore, usedSelectors);
