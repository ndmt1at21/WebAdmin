...
describe('auth_digest', function () {
  describe('With qop (RFC 2617)', function () {
    it('should generate a proper header', function () {
      ...
      var performDigest = function performDigest() {
        ...
        var expectedResponse = md5([ha1, parsedUpdatedHeader.nonce, parsedUpdatedHeader.nc, parsedUpdatedHeader.cnonce, parsedUpdatedHeader.qop, ha2].join(':'));
        return {
      ...
      var result = performDigest();
      result.header.should.match(/qop="auth"/).match(/uri="\/dir\/index.html"/).match(/opaque="5ccc069c403ebaf9f0171e9517f40e41"/).match(/realm="testrealm@host\.com"/).match(/response=/).match(/nc=/).match(/nonce=/).match(/cnonce=/);
  ...
  describe('With plus character in nonce header', function () {
    it('should generate a proper header', function () {
      ...
      var result = performDigest();
      result.header.should.match(/nonce="dcd98b7102dd2f0e8b11d0f6\+00bfb0c093"/);
    });
  });
  describe('Without qop (RFC 2617)', function () {
    it('should generate a proper header', function () {
      ...
      var result = performDigest();
      result.header.should.not.match(/qop=/).match(/uri="\/dir\/index.html"/).match(/opaque="5ccc069c403ebaf9f0171e9517f40e41"/).match(/realm="testrealm@host\.com"/).match(/response=/).not.match(/nc=/).match(/nonce=/).not.match(/cnonce=/);
      result.parsed.response.should.be.eql(result.expectedResponse);
    });
  });
});