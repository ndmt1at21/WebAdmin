...
exports.build = function (data, boundary, callback) {
  if (_typeof(data) != 'object' || typeof data.pipe == 'function') return callback(new Error('Multipart builder expects data as key/val object.'));
  var body = '',
      object = flatten(data),
...
function flatten(object, into, prefix) {
  ...
  for (var key in object) {
    ...
    var prop = object[key];
    if (prop && _typeof(prop) === 'object' && !(prop.buffer || prop.file || prop.content_type)) flatten(prop, into, prefix_key);else into[prefix_key] = prop;
  }
