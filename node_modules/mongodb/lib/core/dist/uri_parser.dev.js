...
var AUTH_MECHANISMS = new Set(['GSSAPI', 'MONGODB-AWS', 'MONGODB-X509', 'MONGODB-CR', 'DEFAULT', 'SCRAM-SHA-1', 'SCRAM-SHA-256', 'PLAIN']); // Lookup table used to translate normalized (lower-cased) forms of connection string
...
function applyConnectionStringOption(obj, key, value, options) {
  ...
  if (key === 'authmechanism' && !AUTH_MECHANISMS.has(value)) {
    throw new MongoParseError("Value for authMechanism must be one of: ".concat(Array.from(AUTH_MECHANISMS).join(', '), ", found: ").concat(value));
  }
...
function parseConnectionString(uri, options, callback) {
  ...
  var directConnection = !!parsedOptions.directConnection;
