...
function (_OperationBase) {
  ...
  function CommandOperationV2(parent, options, operationOptions) {
    ...
    var propertyProvider = _this.hasAspect(Aspect.NO_INHERIT_OPTIONS) ? undefined : parent;
    _this.readPreference = _this.hasAspect(Aspect.WRITE_OPERATION) ? ReadPreference.primary : ReadPreference.resolve(propertyProvider, _this.options);
    _this.readConcern = resolveReadConcern(propertyProvider, _this.options);
    _this.writeConcern = resolveWriteConcern(propertyProvider, _this.options);
    _this.explain = false;
