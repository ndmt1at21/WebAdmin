...
module.exports = function parse(modules, opts) {
  ...
  function finish(src) {
    ...
    (function next() {
      ...
      s.stream.pipe(output, {
        end: false
      });

      if (opts.sourceMap) {
        s.stream.pipe(concat({
          encoding: 'string'
        }, function (chunk) {
  ...
  function walk(node) {
    ...
    } else if (isreqv && node.parent.type === 'MemberExpression' && node.parent.property.type === 'Identifier') {//vars[node.parent.parent.id.name] = varModules[reqid];
    ...
    if (node.parent.type === 'CallExpression') {
      ...
      if (res === undefined && cur.type === 'CallExpression') {
        ...
        if (callee !== undefined) {
          ...
          } catch (err) {// Evaluate to undefined
          }
        }
      }
      ...
      if (res !== undefined) {
        ...
        output.emit('error', new Error('unsupported unary operator: ' + node.parent.operator));
      }
    } else {
...
function isStream(s) {
  return s && _typeof(s) === 'object' && typeof s.pipe === 'function';
}
